 ...
 *  3) ConnectSignUpServer connectSignUpServer = new ConnectSignUpServer();
 *     connectSignUpServer.run(var);
 *     이 두줄로 액티비티에서 실행
 */
public class ConnectSignUpServer {
    ...
    public void run(String name, String email, String password, String educationofficer, String studentnumber, String school_id, String registration_id){
        //Post로 보낼때는 http body에 인코딩해서 보내기때문에 RequstBody에 add(Name, Value)하고 build()
        RequestBody formBody = new FormBody.Builder().add("name", name).add("email", email).add("password", password).add("educationofficer", educationofficer).add("studentnumber",studentnumber).add("school_id",school_id).add("registration_id",registration_id).build();
